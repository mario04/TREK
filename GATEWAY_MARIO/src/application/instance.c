...
//application data message byte offsets, positions in msg vector
...
#define ISMB_MSG                            3               // ISMB Message
#define X_Coord                      (3 + ISMB_MSG_LEN)     // X coordinate of the ISMB Message
#define Y_Coord                      (3 + ISMB_MSG_LEN + Coord_LEN)    // Y Coordinate of the ISMB Message
#define NUM_MEAS                     (3 + ISMB_MSG_LEN + Coord_LEN + Coord_LEN)   // Number of Estimate distances Took
#define EST_GEN                      (3 + ISMB_MSG_LEN + Coord_LEN + Coord_LEN + NUM_MEAS_LEN)    // For the flag if the estimated position is generated
#define ACCURACY                     (3 + ISMB_MSG_LEN + Coord_LEN + Coord_LEN + NUM_MEAS_LEN + EST_GEN_LEN)     // Position for de accuracy value
#define CONNECT_ANCHOR               (3 + ISMB_MSG_LEN + Coord_LEN + Coord_LEN + NUM_MEAS_LEN + EST_GEN_LEN + ACCURACY_LEN)   // Position for message conecctivity
#define NEG_COORD_FLAG               (3 + ISMB_MSG_LEN + Coord_LEN + Coord_LEN + NUM_MEAS_LEN + EST_GEN_LEN + ACCURACY_LEN + CON_LEN)
#define COUNTER                (3 + ISMB_MSG_LEN + Coord_LEN + Coord_LEN + NUM_MEAS_LEN + EST_GEN_LEN + ACCURACY_LEN + CON_LEN + NEG_COORD_FLAG_LENGTH)
// -------------------------------------------------------------------------------------------------------------------
//      Data Definitions
// -------------------------------------------------------------------------------------------------------------------
                    ...
                    else
                    {
                        ...
                        switch(fcode)
                        {
                            ...
                            case RTLS_DEMO_MSG_TAG_LOC:
                            {
								...
								non_user_payload_len = TAG_LOC_MSG_LEN;
								if(inst->mode == LISTENER) //don't process any ranging messages when in Listener mode
								{
									//only enable receiver when not using double buffering
									if(inst->doublebufferon == 0)
									...
									// In the following lines it is copied the incomming data in the special poll message in device memory
									inst->newReportLoc = 1;
									inst->rangeNum = messageData[LOC_RNUM];
									memcpy(&inst->Xint, &(messageData[XLOC_POS]),Coord_LEN);
									memcpy(&inst->Yint, &(messageData[YLOC_POS]),Coord_LEN);
									inst->responseMask = messageData[VRESPLOC];
									inst->validLoc = messageData[VLOC];
									inst->flag_neg = messageData[FLAG_NEG];

									inst->TagAddr = srcAddr[0] & 0x3;
...
uint8 uint8_pow(uint8 a, uint8 b){
	uint8 result = 1;
	for(int i = 0; i < b; i++){
		result = a*result;
	}
...
uint64 instance_get_tag_address(void) //get address from the tag that sends the report of the Location
{
    int instance = 0;
    uint64 x = (uint64) instance_data[instance].relpyAddress[5];
    x |= (uint64) instance_data[instance].relpyAddress[6] << 8;
    x |= (uint64) instance_data[instance].relpyAddress[7] << 16;
    return (x);
}
