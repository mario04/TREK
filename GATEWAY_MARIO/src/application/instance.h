// -------------------------------------------------------------------------------------------------------------------
//
#define RTLS_DEMO_MSG_TAG_LOC               (0X3B)//  File: instance.h - DecaWave header for application level instance
//
...
*******************************************************************************************************************/
#define MAX_NUM_ANCH_SYS (16)  //Maximum Number of anchor in the system
...
#define PUT_TEMP_VOLTAGE_INTO_POLL  (1)     // to insert wakeup sampled TEMP/VOLTAGE into POLL message
...
#define RTLS_DEMO_MSG_TAG_POLL_ISMB         (0xAB)          // Tag poll message ISMB
...
#define TAG_LOC_MSG_LEN                       13
#define ANCHOR_LIST_SIZE			(5) // Number of anchors in system
#define TAG_LIST_SIZE				(1)	//anchor will range with 1st Tag it gets blink from

//lengths including the Decaranging Message Function Code byte and the special fields for Gateway msg
#define ISMB_MSG_LEN                        1               // Length for ISMB Message
#define Coord_LEN                           4               // Length for X coordinate of the ISMB Message
#define NUM_MEAS_LEN                        1               // Length for Number of Estimate distances Took
#define EST_GEN_LEN                         1               // Length for Flag Estimated Position Generated
#define ACCURACY_LEN                        2               // Length for Accuracy message
#define CON_LEN                      (MAX_NUM_ANCH_SYS/8)   // Length for Connected anchor message
#define NEG_COORD_FLAG_LENGTH               1
#define COUNTER_LENGTH                      4
#define TAG_POLL_MSG_LEN                   (3 + ISMB_MSG_LEN + Coord_LEN + Coord_LEN + NUM_MEAS_LEN + EST_GEN_LEN + ACCURACY_LEN + CON_LEN + NEG_COORD_FLAG_LENGTH + COUNTER_LENGTH)  // FunctionCode(1), Temp (1), Volt (1), ISMB MESSAGE
...
typedef struct
{
	...
	uint8 NumMeas; //Number of measurements done
	uint8 EstGen; //Flag is 1 if there is an estimated position, 0 if not
	double Accuracy;//provided by the P matrix, only in EKF
	uint16 Connectivity;//Vector with the position of the anchors communicated
	uint32 Counter;
}msg2gateway;
